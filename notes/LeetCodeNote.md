...
## 十三、栈处理嵌套结构（LeetCode 394 字符串解码）

**问题特征：** 出现"嵌套"、"先开后关"的结构（括号匹配、嵌套表达式、嵌套字符串）→ 用 **Stack**。

### 1. 双 stack 模式

`"3[a2[c]]"` 这种嵌套结构需要保存两类信息：
```java
Stack<Integer> numStack = new Stack<>();  // 存重复次数
Stack<String>  strStack = new Stack<>();  // 存"还没拼完的"前缀
String cur = "";   // 当前正在累积的字符串
int    num = 0;    // 当前正在累积的数字
```
...
| 数字 | `num = num * 10 + (c - '0');` |
| 字母 | `cur += c;` |
| `[`  | push num 和 cur，**清空** num 和 cur |
| `]`  | pop k 和 prev，`cur = prev + cur × k` |
...
num = num * 10 + (c - '0');
```
**原理**：每读一位，把已有数字"左移一位"（×10）腾出位置给新数字。
- `"123"`：1 → 12 → 123
- `"100"`：1 → 10 → 100
- 同时兼容单个数字和多位数字
...
cur = "";      // ← 容易忘
```
进入新块前，必须把当前状态完整保存，再清空准备重新累积。

### 5. `]` 触发"还原 + 合并"

```java
int k = numStack.pop();
String temp = cur;       // 保存原始 cur
cur = "";
for (int j = 0; j < k; j++) cur += temp;   // cur 重复 k 次
cur = strStack.pop() + cur;                 // 拼上外层前缀
```

**易错点**：
- ❌ `cur = cur + cur` —— 这是翻倍（指数爆炸），不是重复 k 次
- ❌ `numStack.pop()` 写在 for 条件里 —— 每次循环都 pop！应先存到变量
- ❌ 用循环外的 prev 变量 —— prev 必须从 strStack pop 出来

### 6. 通用思路："块结构"题目模板

凡是遇到"块状嵌套"（括号、HTML 标签、嵌套数组）：

```
进入块（开符号）：保存当前状态 → 清空准备
处理块内：累积内容
退出块（闭符号）：取出保存的状态 → 合并块内结果
```
